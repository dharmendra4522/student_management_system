...
def STAFF_LEAVE_VIEW(request):
    ...
    return render(request,'Hod/staff_leave.html',contetx)
...
@login_required(login_url='/')
def STAFF_APPROVE_DISLEAVE(request,id):
    leave = Staff_leave.objects.get(id = id)
    leave.status = 2
    leave.save()
    return redirect('staff_leave_view')
...
def STUDENT_APPROVE_DISLEAVE(request,id):
    student_leave = Student_leave.objects.get(id=id)
...
def STAFF_FEEDBACK_SAVE(request):
    if request.method == "POST":
        ...
        feedback.status = 1
        feedback.save()
        messages.success(request,'Reply has been Successfully Send')
